...
def _validate_args(args):
    ...
    # NUMINA validation
    if args.numina:
        assert "t2v" in args.task, (
            f"NUMINA only supports t2v tasks, got: {args.task}"
        )
        assert args.numina_noun_counts is not None, (
            "NUMINA requires --numina_noun_counts, e.g. '{\"cats\": 3, \"dogs\": 2}'"
        )
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable NUMINA numerical alignment for t2v generation.")
    parser.add_argument(
        "--numina_noun_counts",
        type=str,
        default=None,
        help='JSON dict mapping nouns to target counts, e.g. \'{"cats": 3, "dogs": 2}\'')
    parser.add_argument(
        "--numina_reference_step",
        type=int,
        default=20,
        help="[NUMINA] Reference denoising step for attention extraction (0-based).")
    parser.add_argument(
        "--numina_reference_layer",
        type=int,
        default=15,
        help="[NUMINA] Reference DiT layer for attention extraction (0-based).")
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        # --- NUMINA path ---
        if args.numina and "t2v" in args.task:
            logging.info("=" * 60)
            logging.info("[NUMINA] Enabled — numerical alignment mode")
            logging.info("=" * 60)

            from numina.config import NuminaInput
            from numina.token_mapper import map_noun_tokens

            # Parse noun counts from JSON string
            noun_counts = json.loads(args.numina_noun_counts)
            logging.info(f"[NUMINA] Noun-count targets: {noun_counts}")
            ...
            numina_input = NuminaInput.from_noun_counts(
                ...
                seed=args.base_seed,
                reference_step=args.numina_reference_step,
                reference_layer=args.numina_reference_layer,
                total_steps=args.sample_steps,
            )

            # Map noun strings to T5 token indices
            numina_input = map_noun_tokens(numina_input, wan_t2v.text_encoder.tokenizer)

            for noun, target in numina_input.targets.items():
                logging.info(
                    f"[NUMINA]   '{noun}': count={target.target_count}, "
                    f"token_indices={target.token_indices}"
                )

            logging.info("[NUMINA] Generating video with numerical alignment...")
            video = wan_t2v.generate_numina(
                numina_input,
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            numina_tag = "_numina" if args.numina else ""
            args.save_file = f"{args.task}_{args.size.replace('*','x') if sys.platform=='win32' else args.size}_{args.ulysses_size}_{args.ring_size}_{formatted_prompt}_{formatted_time}{numina_tag}" + suffix
